...
import { captureScreen, captureRegion, getScreenSize } from "./pc/screen";
import { getClipboardText, setClipboardText, copySelectedText, pasteText, getClipboardImage } from "./pc/clipboard";
...
const formatValues = Object.values(CaptureFormat);
const mouseButtonValues = Object.values(MouseButton);
...
export const pcControlTools = [
  ...
  },
  {
    ...
    description: getUiElementInfoTool.description,
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: invokeUiElementActionTool.description,
    inputSchema: {
      type: "object",
      properties: {
        windowTitle: { type: "string", description: "Window title must be provided" },
...
export async function handlePcControlTool(name: string, args: any) {
  try {
    switch (name) {
      case "capture_screen": {
        const { format, quality } = args;
        const result = await captureScreen(format, quality);
      ...
      case "capture_region": {
        ...
        const result = await captureRegion(region, format, quality);
      ...
      case "click_mouse": {
        ...
        return { content: [{ type: "text", text: `Mouse clicked with ${button || MouseButton.LEFT} button` }] };
      }
      case "click_at": {
        const { x, y, button } = args;
        const position: Point = { x, y };
        await clickMouseAt(position, button);
        return { content: [{ type: "text", text: `Mouse clicked at position (${x}, ${y}) with ${button || MouseButton.LEFT} button` }] };
      }
      case "double_click": {
        await doubleClick();
      ...
      case "drag_mouse_from_to": {
        ...
        await dragMouseFromTo(start, end);
      ...
      case "type_text_with_delay": {
        ...
        await typeTextWithDelay(text, delayMs);
      ...
      case "get_clipboard_image": {
        const base64Image = await getClipboardImage();
        if (base64Image) {
      ...
      case "get_ui_element_info": {
        const resultJson = await getUiElementInfoTool.execute(args as GetUiElementInfoInput);
