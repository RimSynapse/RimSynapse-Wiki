...
export async function handleProjectTool(name: string, args: any, token: string, defaultProjectId: string) {
    ...
    if (name === "add_project_item") {
        ...
        `;
        const result: any = await graphqlWithAuth(query, { projectId, contentId: args.contentId });
    ...
    if (name === "get_project_items") {
        ...
        `;
        const result: any = await graphqlWithAuth(query, { projectId });
    ...
    if (name === "update_project_item_status") {
        ...
        const result: any = await graphqlWithAuth(mutation, { projectId, itemId: args.itemId, fieldId, optionId });
    ...
    if (name === "update_project_item_iteration") {
        ...
        if (iterName.includes("1")) iterationId = "381c7c80";
        else if (iterName.includes("2")) iterationId = "54cf5c95";
        else if (iterName.includes("3")) iterationId = "d2c335bc";
        else throw new Error(`Unknown iteration: ${args.iterationName}`);
        ...
        const result: any = await graphqlWithAuth(mutation, { projectId, itemId: args.itemId, fieldId, iterationId });
