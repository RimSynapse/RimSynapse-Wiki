...
async function startSprint() {
    ...
    for (const item of items) {
        ...
        if (iteration1Targets.includes(item.content.title)) {
            ...
            await (0, projects_1.handleProjectTool)("update_project_item_iteration", { itemId: item.id, iterationName: "Iteration 1" }, token, config.defaultProjectId);
        }
        else if (backlogTargets.includes(item.content.title)) {
            console.log(`Setting Backlog for: ${item.content.title}`);
            await (0, projects_1.handleProjectTool)("update_project_item_status", { itemId: item.id, status: "Backlog" }, token, config.defaultProjectId);
        }
    }
